...
if (isset($_SESSION['alogin'])) {
	...
	$rid = $rid_row1['rid'];
	$viewbrand = "Select * from tbl_complaints where email='$mail'";
	$d_seller_brand = mysqli_query($con, $viewbrand);
	$viewprod = "SELECT *
	FROM tbl_complaints
	JOIN tbl_customer_order ON tbl_complaints.customer_order = tbl_customer_order.customer_order_id
	JOIN tbl_product ON tbl_complaints.stock_product_id = tbl_product.pid
							...
							?>
								<form action="complaints_replay_action.php?complaintid=<?php echo $prod['complaints_id']; ?>" method="POST">
									...
									<tr>
										...
										<td><?php echo $prod['complaint_message'] ?></td>
									</tr>
									...
									<tr>
										<td colspan="2">
											<center><input type="submit" name="send_reply" id="send_reply" value="Send Response" class="btn btn-danger btn-sm " style="background-color:green;color:lack ;font-weight:bold"> </center>
										</td>
